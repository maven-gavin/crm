...
public class UserRepository2Impl {
    private String findAllQL = "from User u where 1 = 1";
    private String countAllQL = "select count(u) from User u where 1=1";
    ...
    private SearchCallback customSearchCallback = new DefaultSearchCallback() {
        ...
        public void prepareQL(StringBuilder hql, Searchable search) {
            if (search.containsSearchKey("realname")) {
                hql.append(" and exists(select 1 from BaseInfo bi where u = bi.user and bi.realname like :realname )");
            }
        }
